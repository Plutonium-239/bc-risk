...
footer = html.Div([
 "Made by : ",
 html.A("Yukti Makhija", href='https://github.com/yuktimakhija'), " and ",
 html.A("Samarth Bhatia", href='https://plutonium-239.github.io/')
],
...
maincontent = dbc.Container([
 dbc.Row(
  dbc.Col([
   ...
   dbc.Tabs([
    dbc.Tab([
     contents.any_content(),
     dbc.Button('Predict Mutation of Any Gene', href='/any_gene')
    ],
            label='Any Gene Mutation'),
    dbc.Tab([
     contents.imp_content(),
     dbc.Button('Predict Mutation of Pathogenic Gene', href='/path_gene')
    ],
            label='Pathogenic Gene Mutation'),
    dbc.Tab([
     contents.vus_content(),
     dbc.Button('Predict Variant of Unknown Significance', href='/vus')
    ],
            label='VUS Mutation')
   ], ),
  ],
...
@app.callback(Output('any_output', 'children'), [Input('any_table', 'data')])
def any_table(data):
	...
	return [
	 html.H3([pos]),
	 html.Br(),
	 html.H1(str(str(round(prob[0, 1] * 100, 3)) + "%"))
	]


@app.callback(Output('imp_output', 'children'), [Input('imp_table', 'data')])
def imp_table(data):
...
@app.callback(Output('vus_output', 'children'), [Input('vus_table', 'data')])
def vus_table(data):
